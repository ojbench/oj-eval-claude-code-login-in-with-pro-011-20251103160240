...
private:
	...
	// Merge two leftist heaps - may throw if cmp throws
	Node* mergeNodes(Node *h1, Node *h2) {
		...
		// For max heap, we want larger values at root
		// So if cmp(h1->data, h2->data) is true, h1 < h2, swap them
...
public:
	...
	void push(const T &e) {
		...
		try {
			newNode = new Node(e);
			// Try to merge - this may throw if cmp throws
			Node *oldRoot = root;
	...
	void pop() {
		...
		try {
			...
			// Restore state - this shouldn't happen in pop normally
			// but we need to handle it for exception safety
			root = oldRoot;
			throw runtime_error();
		}
	}
